...
async def check_type_e_redundancy(conn: asyncpg.Connection, instance_id: str) -> bool:
    sql = """
        SELECT EXISTS (
          SELECT 1
          FROM resource_relationships rr
          JOIN resources r ON r.instance_id = rr.resource_id
    ...
    """
    return bool(await conn.fetchval(sql, instance_id))
...
async def get_upstream_callers(
    ...
    sql = """
        ...
        ORDER BY depth ASC
    """
    rows = await conn.fetch(sql, instance_id, max_depth)
    return [dict(row) for row in rows]


async def count_active_lb_targets(conn: asyncpg.Connection, lb_instance_id: str) -> int:
    sql = """
        SELECT COUNT(DISTINCT rr.related_resource_id)
        FROM resource_relationships rr
        JOIN resources r ON r.instance_id = rr.related_resource_id
        WHERE rr.resource_id = $1
          AND rr.relationship_type IN ('routes_traffic_to', 'load_balances_to')
          AND r.status = 'active'
          AND rr.related_resource_id != $1
    """
    value = await conn.fetchval(sql, lb_instance_id)
...
async def write_waste_rows(conn: asyncpg.Connection, rows: list[dict]) -> None:
    insert_sql = """
        ...
        ON CONFLICT (instance_id, waste_type)
        DO UPDATE SET
    ...
    tuples = [
        (
            row["instance_id"],
            row["waste_type"],
            str(row["detection_window"]) if row["detection_window"] is not None else None,
            row["avg_cpu"],
            row["peak_cpu"],
            row["avg_ram"],
