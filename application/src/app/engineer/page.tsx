...
type Tab = 'phase1' | 'phase2' | 'llm' | 'prs';
...
function Spinner({ color = 'purple' }: { color?: string }) {
  return (
    <div className="flex items-center justify-center h-64">
      <div className={`animate-spin w-6 h-6 border-2 border-${color}-500 border-t-transparent rounded-full`} />
    </div>
  );
}
...
export default function EngineerInterface() {
  ...
  // Load phase + LLM data whenever selected run changes
  useEffect(() => {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-400 text-sm mt-1">
          Pipeline details, guardrail decisions, LLM analysis, and infrastructure PRs
        </p>
      </div>
      ...
      {selectedRunId && (
        <>
          ...
          {phaseLoading ? (
            ...
            <>
              {activeTab === 'phase1' && phases && (
                <Phase1Panel ec2={phases.ec2Phase1} s3={phases.s3Phase1} />
              )}
              {activeTab === 'phase2' && phases && (
                <Phase2Panel data={phases.ec2Phase2} />
              )}
              {activeTab === 'llm' && llmData && (
                <LLMReportPanel ec2Waste={llmData.ec2Waste} s3Waste={llmData.s3Waste} />
              )}
              {activeTab === 'prs' && (
                <PRListPanel prs={prs} error={prsError} />
              )}
            </>
          )}
        </>
